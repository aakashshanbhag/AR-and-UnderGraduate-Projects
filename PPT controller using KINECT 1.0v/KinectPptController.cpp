//Presentation Control Using Kinect
//------------------------------------------------------------------------------
// <copyright file="SkeletonBasics.cpp" company="Microsoft">
//     Copyright (c) Microsoft Corporation.  All rights reserved.
// </copyright>
// Partial Authors: Aakash Shanbhag, Anusha Relwani,Kesha Bodawala.
//------------------------------------------------------------------------------
...
void Setleftmove(BOOL bstate2);
void Setrightmove(BOOL bstate1);
/// <summary>
/// Draws a skeleton
/// </summary>
...
{      
	...
	//Program begins here
	int Ar = m_Points[NUI_SKELETON_POSITION_WRIST_RIGHT].x - m_Points[NUI_SKELETON_POSITION_HEAD].x;
	int Br = m_Points[NUI_SKELETON_POSITION_WRIST_RIGHT].y - m_Points[NUI_SKELETON_POSITION_HEAD].y;
	int Cr = m_Points[NUI_SKELETON_POSITION_WRIST_RIGHT].x - m_Points[NUI_SKELETON_POSITION_SHOULDER_LEFT].x;
	int hr = m_Points[NUI_SKELETON_POSITION_HIP_CENTER].y- m_Points[NUI_SKELETON_POSITION_SHOULDER_CENTER].y;
	int wr = m_Points[NUI_SKELETON_POSITION_WRIST_RIGHT].x -m_Points[NUI_SKELETON_POSITION_SHOULDER_CENTER].x;
	int Dr = m_Points[NUI_SKELETON_POSITION_WRIST_RIGHT].x - m_Points[NUI_SKELETON_POSITION_SHOULDER_CENTER].x;

	int Al = -(m_Points[NUI_SKELETON_POSITION_WRIST_LEFT].x - m_Points[NUI_SKELETON_POSITION_HEAD].x);
	int Bl = m_Points[NUI_SKELETON_POSITION_WRIST_LEFT].y - m_Points[NUI_SKELETON_POSITION_HEAD].y;
	int Cl = -(m_Points[NUI_SKELETON_POSITION_WRIST_LEFT].x - m_Points[NUI_SKELETON_POSITION_SHOULDER_RIGHT].x);
	int hl = m_Points[NUI_SKELETON_POSITION_HIP_CENTER].y - m_Points[NUI_SKELETON_POSITION_SHOULDER_CENTER].y;
	int wl = -(m_Points[NUI_SKELETON_POSITION_WRIST_LEFT].x - m_Points[NUI_SKELETON_POSITION_SHOULDER_CENTER].x);
	int Dl = -(m_Points[NUI_SKELETON_POSITION_WRIST_LEFT].x - m_Points[NUI_SKELETON_POSITION_SHOULDER_CENTER].x);

	float rl1 = (float)Cl / hl;
	float rl2 = (float)Al / hl;
	float rl3 = (float)Bl / hl;
	float rl4 = (float)Cl / hl;


	float rr1 = (float)Cr / hr;
	float rr2 = (float)Ar / hr;
	float rr3 = (float)Br / hr;
	float rr4 = (float)Cr / hr;
	...
	if (flag == 1)
		...
		count = 0;
		if (flag == 1 &&  rr2<1.8 && rr3>0.45 && rr4<1.9)
	{
	}
	else
	{
...
//for left hand 
	...
	if (flag == 1)
		...
		count = 0;
	if (flag == 1 && rl2<1.9 && rl3>0.35 && rl4<2)
	{
	}
	else
	{
...
void Setrightmove(BOOL bState1)
{
	...
	if ((bState1 && !(keyState[VK_RIGHT] & 1)) ||
		(!bState1 && (keyState[VK_RIGHT] & 1)))
	{
...
void Setleftmove(BOOL bState2)
{
	...
	GetKeyboardState((LPBYTE)&keyState);
	if ((bState2 && !(keyState[VK_LEFT] & 1)) ||
		(!bState2 && (keyState[VK_LEFT] & 1)))
	{
		// Simulate a key press
		keybd_event(VK_LEFT,
			0x25,
